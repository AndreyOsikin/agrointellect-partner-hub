...
(function(){
  ...
  function applyCms(data){
    ...
    if (Array.isArray(data.integrations) && typeof INTEGRATIONS !== 'undefined') {
      ...
      data.integrations.filter(r=>bool(r['Активно'])).forEach(r=>{const group=text(r['Группа']);if(!group)return;if(!groups.has(group))groups.set(group,{group,desc:text(r['Описание']),items:[]});const system=text(r['Система']);if(system)groups.get(group).items.push(system);});
